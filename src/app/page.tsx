      ...
      <div className={`flex h-auto min-h-screen w-full flex-col bg-background-light ${!isOpened ? 'overflow-hidden h-screen' : ''}`}>
            ...
            <div className="px-6 space-y-12 max-w-sm mx-auto">
              ...
              <div className="text-center space-y-6 animate-fade-in-up">
                <div className="relative size-56 mx-auto group">
                  ...
                  <div
                    className="w-full h-full rounded-full bg-center bg-cover border-4 border-white shadow-2xl overflow-hidden transform group-hover:rotate-3 transition-transform"
                    style={{ backgroundImage: "url('/groom.png')" }}
                  ></div>
                </div>
                <div className="space-y-3">
                  <h3 className="text-primary text-3xl font-black italic">Aris S.T.</h3>
                  <p className="text-slate-500 text-sm leading-relaxed px-4 italic">
                    Putra Kedua dari <br />
                    <span className="font-bold text-primary not-italic">Bapak Aris Sr. & Ibu Aris Sr.</span>
                  </p>
                </div>
              </div>
              ...
              <div className="text-center space-y-6 animate-fade-in-up [animation-delay:0.5s]">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed px-4 italic">
                    ...
                    <span className="font-bold text-primary not-italic">Bapak Maya Sr. & Ibu Maya Sr.</span>
                  </p>
                </div>
              </div>
            </div>
        ...
        <section id="details" className="min-h-screen py-16 border-t border-primary/5">
          ...
          >
            ...
            <div className="animate-zoom-in [animation-delay:0.2s]">
              <Countdown />
            </div>
            ...
            <div className="px-4 space-y-8 mt-12">
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 italic text-[10px]">Mohon hadir 15 menit lebih awal</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 italic text-[10px]">Cocktails, Makan Malam & Hiburan</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
              ...
              <div className="bg-white/40 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-primary/5 space-y-6 animate-fade-in-up [animation-delay:0.5s]">
                <div className="flex items-center gap-3 mb-2">
                  <div className="bg-accent text-white p-2 rounded-lg"><span className="material-symbols-outlined icon-filled">map</span></div>
                ...
                <a href="https://www.google.com/maps/search/?api=1&query=St.+Mary%27s+Botanical+Garden" target="_blank" rel="noopener noreferrer" className="w-full bg-primary text-white h-14 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-primary/90 transition-all shadow-lg active:scale-95">
                  <span className="material-symbols-outlined">directions</span> Buka di Google Maps
                </a>
              </div>

              {/* Dresscode Card */}
              <div className="bg-white/40 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-primary/5 space-y-6 animate-fade-in-up [animation-delay:0.6s]">
                <div className="flex items-center gap-3 mb-2">
                  <div className="bg-sage text-white p-2 rounded-lg"><span className="material-symbols-outlined icon-filled">checkroom</span></div>
                ...
                <p className="text-slate-500 text-sm leading-relaxed text-center italic">
                  Kami sangat menghargai kehadiran Anda dalam balutan busana bernuansa <span className="font-bold text-primary not-italic">Rustic & Earthy</span>.
                </p>
                <div className="flex justify-center gap-3 py-2">
                  {[{ color: "bg-sage", name: "Sage" }, { color: "bg-terracotta", name: "Terracotta" }, { color: "bg-tan", name: "Tan" }, { color: "bg-chocolate", name: "Brown" }, { color: "bg-ivory", name: "Ivory" }].map((item, idx) => (
                    <div key={item.name} className="flex flex-col items-center gap-2 animate-zoom-in" style={{ animationDelay: `${0.7 + idx * 0.1}s` }}>
                      <div className={`${item.color} size-10 rounded-full shadow-inner border border-black/5 hover:scale-110 transition-transform`} />
          ...
          >
            ...
            <div className="px-4 py-8 masonry-grid">
              {[
                ...
                <div key={i} className="masonry-item animate-zoom-in" style={{ animationDelay: `${i * 0.15}s` }}>
                  <img src={src} alt="Gallery" className="w-full rounded-2xl shadow-lg border border-white/20 hover:scale-[1.03] transition-transform duration-500" />
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            ...
            {isSubmitted ? (
              <div className="px-6 py-12 text-center space-y-4 animate-zoom-in">
                <div className="bg-primary/10 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce"><span className="material-symbols-outlined text-4xl text-primary icon-filled">check_circle</span></div>
                ...
                <button onClick={() => setIsSubmitted(false)} className="text-primary font-bold text-xs underline underline-offset-4 pt-4 active:scale-95 transition-transform">Kirim pesan lain</button>
              </div>
            ) : (
              <form className="px-6 space-y-6 max-w-sm mx-auto animate-fade-in-up [animation-delay:0.2s]" onSubmit={async (e) => { e.preventDefault(); if (!rsvpName || isAttending === null) return; setIsSubmitting(true); try { const res = await fetch('/api/rsvp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: rsvpName, attending: isAttending, message: rsvpMessage }) }); if (res.ok) { setIsSubmitted(true); setRsvpMessage(""); fetchResponses(); } } catch (error) { console.error(error); } finally { setIsSubmitting(false); } }}>
                <div className="space-y-2">
                  ...
                  <input type="text" required value={rsvpName} onChange={(e) => setRsvpName(e.target.value)} className="w-full h-14 bg-white/40 border border-primary/10 rounded-xl px-4 focus:ring-2 focus:ring-primary/20 outline-none transition-all placeholder:italic" placeholder="Enter your guest name" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-primary">Will you attend?</label>
                  <div className="grid grid-cols-2 gap-4">
                    <button type="button" onClick={() => setIsAttending(true)} className={`h-14 rounded-xl border-2 transition-all font-bold ${isAttending === true ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20 scale-105' : 'border-primary/10 text-primary hover:bg-primary/5'}`}>Yes, I&apos;m In!</button>
                    <button type="button" onClick={() => setIsAttending(false)} className={`h-14 rounded-xl border-2 transition-all font-bold ${isAttending === false ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20 scale-105' : 'border-primary/10 text-primary hover:bg-primary/5'}`}>Sorry, can&apos;t</button>
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-primary">Ucapan & Doa</label>
                  <textarea value={rsvpMessage} onChange={(e) => setRsvpMessage(e.target.value)} className="w-full h-32 bg-white/40 border border-primary/10 rounded-xl p-4 focus:ring-2 focus:ring-primary/20 outline-none transition-all resize-none placeholder:italic" placeholder="Tuliskan ucapan & doa untuk mempelai..." />
                </div>
                <button type="submit" disabled={isSubmitting || !rsvpName || isAttending === null} className="w-full bg-primary text-white h-16 rounded-2xl font-black text-xl shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all mt-4 disabled:opacity-50 disabled:grayscale disabled:scale-100 italic tracking-wider">
                  {isSubmitting ? 'SENDING...' : 'CONFIRM RSVP'}
                </button>
              </form>
            )}

            <div className="mt-12 px-6 max-w-sm mx-auto animate-fade-in [animation-delay:0.5s]">
              <h3 className="text-primary font-bold mb-4 flex items-center gap-2"><span className="material-symbols-outlined text-sm">forum</span>Guest Messages ({responses.length})</h3>
              <div className="space-y-4 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar pb-8">
                {responses.length === 0 ? (
                  <p className="text-primary/40 italic text-center py-8 text-sm bg-white/10 rounded-2xl border border-dashed border-primary/20 animate-pulse">Belum ada pesan.</p>
                ) : (
                  responses.map((resp, i) => (
                    <div key={resp.id} className="bg-white/40 backdrop-blur-sm p-4 rounded-2xl border border-primary/5 space-y-2 animate-fade-in-up" style={{ animationDelay: `${0.6 + i * 0.1}s` }}>
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="font-bold text-primary text-sm flex items-center gap-1">{resp.name}{resp.attending ? <span className="text-[10px] bg-green-500/10 text-green-600 px-2 py-0.5 rounded-full font-black uppercase tracking-tighter">Attending</span> : <span className="text-[10px] bg-red-500/10 text-red-600 px-2 py-0.5 rounded-full font-black uppercase tracking-tighter">Absent</span>}</h4>
                        <span className="text-[10px] text-primary/40 font-medium whitespace-nowrap">{new Date(resp.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</span>
                      </div>
        ...
        <section id="gift" className="min-h-screen py-16 bg-primary text-white">
          ...
          >
            ...
            <div className="px-6 space-y-6">
              <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-8 text-primary shadow-2xl relative overflow-hidden animate-zoom-in [animation-delay:0.3s] group hover:rotate-1 transition-transform">
                ...
                <button className="w-full h-12 bg-primary text-white rounded-xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-transform">
                  <span className="material-symbols-outlined text-sm">content_copy</span> Copy Account Number
                </button>
              </div>

              <div className="bg-white/10 backdrop-blur-md rounded-3xl p-8 border border-white/20 text-center animate-zoom-in [animation-delay:0.5s]">
                <div className="w-48 h-48 bg-white mx-auto rounded-2xl p-3 mb-4 shadow-inner transform hover:scale-105 transition-transform">
                  <img src="https://lh3.googleusercontent.com/aida-public/AB6AXuBZfJNhigGudx9iY9PC_V1CMK1nRikx20hGUF83mYUCbh1_5CKT92_CUFjlN1Fb_TyvqLgIhBTwvbLE5qKC9Cg41fATVeXagtZVLGmL7Gs1FkbKzp-BC03bYqhx-p5O9GkujcgJ89JjxsCXs_Xs0tbRlHMN1nU8TpH8A2EKln3FGe_066eybR-0IC9xrvZ9-nEQUrfmaqdwLSlIw8VO-itjZipeChEryjBDKL7NY-S3asUOdgK1_kuhtETUQPit1aCR4XNJybxgQgM" className="w-full h-full grayscale hover:grayscale-0 transition-all" alt="QRIS" />
                </div>
                <p className="text-xs font-bold uppercase tracking-widest text-white/60 mb-2 italic">Scan QRIS to Donate</p>
              </div>
            </div>

            <div className="pt-24 pb-32 text-center space-y-4 animate-fade-in [animation-delay:0.8s]">
              <div className="flex justify-center gap-4 text-white/40 italic">
...
export default function Home() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-background-light animate-pulse"></div>}>
      <InvitationContent />
    </Suspense>
  );
}
